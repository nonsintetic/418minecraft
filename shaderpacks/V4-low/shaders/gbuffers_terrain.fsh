...
//////////////////////////////ADJUSTABLE VARIABLES
	...
	#define TORCH_INTENSITY 2.6					//torch light intensity /2.0 is default
...
void main() {	
	...
	*/
	...
	sunlight_color = mix(sunlight_color,vec3(0.15),rainStrength*0.9);
	...
	ambient_color = mix(ambient_color,vec3(0.15),rainStrength*0.9);
	
	float sky_atten = min(lmcoord.t*lmcoord.t*lmcoord.t+0.01,1.0);

	
	vec3 lightcol = mix(pow(ambient_color,vec3(2.0)),lightcolor,sky_atten*sky_atten)*sky_atten;
	
	float torchlight = lmcoord.s* lmcoord.s* lmcoord.s*TORCH_INTENSITY;
	
	vec3 pixcolor = sqrt(albedo*(lightcol+torchlight*torchcolor));
