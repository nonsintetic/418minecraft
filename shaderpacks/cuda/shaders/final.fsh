...
// depth of field
    ...
    float depth = texture2D(depthtex2, texcoord.st);

    float depth_fading = centerDepthSmooth;
    float focus = (depth - depth_fading) / 5;	
	...
	for ( int i = 0; i < 60; i++) {
	    if (isEyeInWater > 0.99) {
		    sample = texture2D(gaux2, texcoord.xy + hex_offsets[i]/300*vec2(1.0,aspectRatio));
		    bcolor += sample.rgb;
		} else {
		    sample = texture2D(gaux2, texcoord.xy + hex_offsets[i]*focus + hex_offsets[i]/vignette_blur/1000*blurStrength*vec2(1.0,aspectRatio));
		    bcolor += sample.rgb;
		}
	}
